#!/usr/bin/env python3
"""
Risk Arbiter — runs every 30 seconds via APScheduler.
...
This script is the ONLY process allowed to set RISK_OFF.
"""
...
def get_account_equity() -> float | None:
    """Fetch current account equity via Vulcan paper status."""
    status = vulcan_paper_status()
    if isinstance(status, dict) and "error" in status:
        log(f"Arbiter equity error: {status.get('error')}")
        return None
...
def flatten_all():
    """Emergency: close every open position via Vulcan paper."""
...
def main():
    ...
    try:
        ...
        # Respect manual lock — don't override in paper mode
        if regime.get("riskModeLocked"):
            # Still track equity for observability
        ...
        # Time-based RISK_OFF auto-clear
                    ...
                    if hours_elapsed >= 24:
                        if equity > 0 and peak > 0:
                            ...
                            if current_drawdown < guardrails.get(
                                ...
                                log(
                                    f"Arbiter: auto-cleared RISK_OFF -> BASELINE (24h elapsed)"
                                )
            ...
            if peak_drawdown >= catastrophic_pct:
                ...
                if already_flattened:
                    if arb_state.get("flattenedAt") is None:
                        ...
                        save_arbiter_state(arb_state)
                    log(
                        f"RISK ARBITER: CATASTROPHIC persists but already flattened — skipping"
                    )
                else:
                    log(
        ...
        if recent_stops >= max_stop_outs:
            if regime.get("riskMode") != "RISK_OFF":
                ...
                send_telegram(
                    f"RISK OFF — {recent_stops} consecutive stop-outs\n"
            ...
            for strat in strategies:
                ...
                for pos in positions:
                    ...
                    if pos_pnl == 0:
                        ...
                        leverage = max(1, pos.get("leverage", 1))
                        if current_roe is not None and entry_price > 0:
                    ...
                    if pos_pnl < 0 and abs(pos_pnl) > equity * single_loss_pct / 100:
                        ...
                        if isinstance(result, dict) and result.get("error"):
                            log(
                                f"RISK ARBITER: Close FAILED for {asset}: {result.get('error')}"
                            )
                            send_telegram(
                                f"Single trade close FAILED for {asset}\n"
                                f"Error: {result.get('error')}\n"
                                f"Position may still be open — manual close required"
                            )
                            continue
