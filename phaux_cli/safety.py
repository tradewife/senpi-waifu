"""
safety.py — Centralized entry gate pipeline for the phaux system.
...
def evaluate_entry(entry: dict, strategy: dict) -> GateResult:
    ...
    # Gate 3: Valid strategy (local config — just needs name + enabled)
    strat_name = strategy.get("name", "")
    result.add(
        bool(strat_name) and strategy.get("enabled", False),
        "No valid strategy configured",
    )
    ...
    # Gate 5: Scanner not blocked by brain policy (static policy engine)
    ...
    # Gate 6: Score threshold (user-configurable)
    ...
    # Gate 8: Per-asset cooldown (HARD — non-negotiable)
    result.add(
        not check_hard_cooldown(asset),
        f"Asset {asset} in cooldown",
    )
