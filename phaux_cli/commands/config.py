"""
config.py — Environment configuration management for Phaux paper trading.

Usage:
    phaux config show              Show all config (secrets masked)
    phaux config get <key>         Get specific value
    phaux config set <key> <val>   Set value (writes to .env)
    phaux config validate          Check required vars are set
    phaux config export            Export as .env format
    phaux config rules             Display current user rules (thresholds)
"""
...
REQUIRED_VARS = [
    ("PHAUX_DIR", "Phaux project root directory"),
]

OPTIONAL_VARS = [
    ("PHAUX_DIR", str(PROJECT_ROOT), "Project root directory"),
...
def show_config():
    ...
    env_file = _load_env_file()
    click.echo(f"\n{'=' * 55}")
    click.echo("  PHAUX CONFIGURATION")
    ...
    if missing:
        ...
        click.echo("   Run: phaux config set <key> <value>")
    else:
...
def get_value(key):
    ...
    if key in SENSITIVE_VARS:
        ...
        click.echo("(masked — use phaux config show to see source)")
    else:
...
def validate_config():
    ...
    if errors:
        ...
        for err in errors:
            click.echo(f"   - {err}")
        click.echo("\n   Fix with: phaux config set <key> <value>")
    else:
